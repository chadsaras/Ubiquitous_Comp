...
\begin{center}
  ...
  \vspace{0.3em}
  {\normalsize
    \mbox{\textbf{Saras Wagh} (22CS30048)} \enspace\textbar\enspace
    \mbox{\textbf{Hemant} (22CS30029)} \enspace\textbar\enspace
    \mbox{\textbf{R Mohan Poornachandra} (23EX10027)}\par
  }
  \vspace{0.8em}
...
\midrule
\textbf{Hemant} & \textbf{Track C Lead (Evaluation) \& Track A Co-Lead (Data \& Cloud Acceleration):} Built raw burst caching infrastructure (\texttt{scripts/build\_raw\_cache.py}); configured Kaggle cloud environment with NVIDIA Tesla T4 GPU; developed interactive training notebooks and executed 5-fold cross-validation for CNN+GRU; led benchmark evaluation harness execution (\texttt{scripts/evaluate\_qa.py}), scoring evaluation, and empirical metric validation across question types. \\
\midrule
\textbf{R Mohan Poornachandra} & \textbf{Track B Lead (Aggregation \& Query Layer) \& Track C Co-Lead (Evaluation \& Report):} Built ingestion engine and unit normalization (\texttt{load.py}); designed Contract 1 (\texttt{Timeline}) and Contract 2 (\texttt{FormattedAnswerBlock}); implemented 32-rule regex intent engine and Ollama Qwen 2.5 explainer (\texttt{src/query/}); co-developed 5-fold benchmark harness (\texttt{evaluate\_qa.py}), scoring rules, robustness sweeps, and authored technical report. \\
\bottomrule
\end{tabular}%
}
\end{table}
