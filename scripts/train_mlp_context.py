#!/usr/bin/env python3
"""
Final accuracy attempt: FeatureMLP with multi-minute temporal context.

Every model so far (RF, HGB, TinyCNN, CNN+GRU, FeatureMLP) classifies one
labelled minute in total isolation, even though activities obviously persist
across neighbouring minutes. This model concatenates each minute's own 302
engineered features with the MEAN of its temporal neighbours' features (up to
2 minutes before + 2 minutes after, only counting a neighbour if it is
actually close in time -- a single gap >90s in the chain stops that
direction, so we never blend in an unrelated, far-away minute). Input is
604-dim (own 302 + context 302) instead of FeatureMLP's 302.

    python scripts/train_mlp_context.py                # CPU, all 5 folds

Context is built from a user's OWN other minutes only, so it never crosses
between users, and since folds split by user, a neighbour minute is always in
the same train/val/test partition as the anchor minute for that fold -- no
leakage. Context is computed from ALL of a user's minutes regardless of the
"clean" flag (a mislabelled minute's raw signal is still real, useful
smoothing signal) -- only the anchor's own label-noise cleaning is affected
by --no-clean, same policy as every other script here.

Prerequisite: data/processed/features.npz (already built, same cache RF/
FeatureMLP train on).

Outputs (results/): confusion_matrix_mlpctx.png, classifier_metrics_mlpctx.json,
model_mlpctx.pt -- same schema as the other classifier_metrics_*.json files.
"""
...
CTX_K = 2            # up to 2 minutes before + 2 after
CTX_GAP_SEC = 90.0    # a chain gap bigger than this stops that direction (matches MERGE_GAP_SEC elsewhere)
...
def add_context(Xm: np.ndarray, um: np.ndarray, tm: np.ndarray,
                gap_sec: float = CTX_GAP_SEC, k: int = CTX_K) -> tuple[np.ndarray, np.ndarray]:
    """
    For each minute, mean-average up to k neighbours before + k after (only
    consecutive ones within gap_sec of each other -- one big gap stops that
    direction). Falls back to the minute's own features when isolated (no
    usable neighbour on either side). Returns (context (n, f), n_neighbours_used (n,)).
    """
    n, f = Xm.shape
    ctx = np.zeros((n, f), dtype=np.float32)
    n_used = np.zeros(n, dtype=int)
    order = np.lexsort((tm, um))          # sort by user, then by time within user
    i = 0
    while i < len(order):
        j = i
        while j < len(order) and um[order[j]] == um[order[i]]:
            j += 1
        idxs = order[i:j]                 # this user's minute indices, sorted by time
        for pos in range(len(idxs)):
            neigh = []
            p = pos
            for _ in range(k):             # walk backward
                if p == 0 or tm[idxs[p]] - tm[idxs[p - 1]] > gap_sec:
                    break
                p -= 1; neigh.append(idxs[p])
            p = pos
            for _ in range(k):             # walk forward
                if p == len(idxs) - 1 or tm[idxs[p + 1]] - tm[idxs[p]] > gap_sec:
                    break
                p += 1; neigh.append(idxs[p])
            idx = idxs[pos]
            if neigh:
                ctx[idx] = Xm[neigh].mean(axis=0)
                n_used[idx] = len(neigh)
            else:
                ctx[idx] = Xm[idx]
                n_used[idx] = 0
        i = j
...
def main() -> None:
    ...
    print(f"device: {device}  arch: mlpctx (FeatureMLP + multi-minute temporal context)")
    ...
    Xs = Xs[:, fmask]
    print(f"{len(ys)} minutes; {Xs.shape[1]} own features per minute; per class: {np.bincount(ys, minlength=7).tolist()}")

    t0 = time.time()
    ctx, n_used = add_context(Xs, us, tss)
    Xc = np.hstack([Xs, ctx]).astype(np.float32)
    n_in = Xc.shape[1]
    print(f"context built in {time.time()-t0:.1f}s -> {n_in} features/minute "
          f"(own {Xs.shape[1]} + context {ctx.shape[1]}); "
          f"isolated minutes (0 usable neighbours): {(n_used == 0).sum()} of {len(n_used)} "
          f"({(n_used == 0).mean():.1%}); mean neighbours used: {n_used.mean():.2f}")
    ...
    for k in args.folds:
        ...
        model, hist = train_one_model(Xc[tr_mask], ys[tr_mask], Xc[val_mask], ys[val_mask], device, n_in,
                                      args.epochs, args.batch_size, args.lr, args.weight_decay,
                                      args.patience, args.seed + k)
        yp = predict(model, Xc[te_mask], device)
        yt = ys[te_mask]
    ...
    plot_cm(cm, out / "confusion_matrix_mlpctx.png",
            "Activity confusion matrix (row-normalised), FeatureMLP+context,\n5-fold user-level CV")
    ...
    final_model, final_hist = train_one_model(Xc[fit_mask], ys[fit_mask], Xc[val_mask], ys[val_mask], device, n_in,
                                              args.epochs, args.batch_size, args.lr, args.weight_decay,
                                              args.patience, args.seed)

    mpath = out / "model_mlpctx.pt"
    torch.save({"state_dict": final_model.state_dict(), "arch": "FeatureMLP+context", "n_in": n_in,
               "ctx_k": CTX_K, "ctx_gap_sec": CTX_GAP_SEC, "classes": CLASSES,
               "feature_mask": fmask.tolist(), "feature_names": [str(n) for n in names]}, mpath)
    ...
    (out / "classifier_metrics_mlpctx.json").write_text(json.dumps({
        "config": vars(args) | {"clean": clean, "arch_name": "FeatureMLP+context", "n_in": n_in,
                                "ctx_k": CTX_K, "ctx_gap_sec": CTX_GAP_SEC,
    ...
    print(f"\nsaved {out / 'classifier_metrics_mlpctx.json'}")
