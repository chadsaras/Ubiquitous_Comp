...
Phase 0 and Phase 1 are fully done. Phase 2 (recognition backbone) is fully done, with **six models trained and evaluated end to end** (two tree ensembles, two raw-signal neural nets, and two engineered-feature neural nets), all on the identical official 5-fold split and the identical set of minutes — full results below in Phase 2, and the final model pick is closed (Step 12: **Random Forest deployed**, FeatureMLP+Context integrated and available but not default). Phase 3 (aggregation/timeline) is built and verified running end to end. Phase 4 (question-answering interface) is built (`src/query/`, all four tiers) and verified running with a real local SLM (Qwen 2.5 1.5B via Ollama), including two real bugs found and fixed by testing it live. Phase 5 (evaluation harness) is **in progress** — see `TASKS.md`, which is the live checklist and supersedes the phase ordering below. Phases 6–8 not started.

**Important correction found while starting Phase 5:** `tests/fixtures/00EABED2_590_160/recording.csv` is **synthetic**, generated by `scripts/generate_test_recording.py` from `truth.json` (only two distinct timestamp deltas in the whole file — a flawless 25 Hz plus one gap; `acc_z` mean exactly the generator's hardcoded 0.98 baseline). Any accuracy measured on it (e.g. an early 93.7%) reflects performance on idealised sine waves, not on ExtraSensory. The real benchmark is being rebuilt from genuine held-out ExtraSensory bursts (`scripts/build_eval_benchmark.py`).
...
**12. Final recognizer pick — Random Forest is the deployed default. FeatureMLP + Context wins the offline benchmark but is not the deployed model. Closed.**

*Offline (per-minute, ExtraSensory-native) benchmark:* FeatureMLP + Context wins on every pooled metric against Random Forest (0.492 vs 0.472 accuracy, 0.431 vs 0.402 macro-F1) and on 4 of 5 individual folds, while being ~290x smaller on disk (0.77 MB vs 223 MB).

*Deployment decision, made after actually integrating it into the live pipeline:* Random Forest is the default that `src/pipeline.py` and `src/aggregate/timeline.py` use. FeatureMLP + Context was fully integrated (a separate `window_predictions_torch()` path in `timeline.py`, selectable with `--model results/model_mlpctx.pt`; RF/HGB code paths untouched and verified unchanged) but underperformed RF badly on a whole-recording run. Two causes, one confirmed and one suspected:
  - *Confirmed:* the model's own official per-class numbers show it over-predicts `lying_down` (precision 0.506). Fed identical, correctly-scaled feature vectors for a known-`sitting` span, RF answered sitting (0.521) while FeatureMLP+Context favoured lying_down (0.472) — the bias is in the model, not the integration.
  - *Suspected, not yet isolated:* its multi-minute context step assumes ExtraSensory's discrete one-burst-per-minute structure. Applied to a whole recording, "neighbouring minute" has to be approximated, which weakens exactly the mechanism that made it win offline.

Both models stay available via `--model`. Revisit once the held-out real-data QA benchmark exists (see `TASKS.md`), which is the first evaluation able to compare them fairly end-to-end.

*Integration bug found and fixed during this work, worth citing under "correctness":* the first version of the torch inference path computed each minute's "own" features by averaging over a flat 60 s window, whereas the model was trained on ~20 s ExtraSensory bursts (RF emulates this correctly with a 7-window sliding aggregation). Feeding 60 s-scale statistics to a network whose BatchNorm layers were calibrated on 20 s-scale statistics produced worse-than-random output (1.8%). Fixed by reusing RF's proven short-span aggregation for the "own" half and confining minute-scale bucketing to the context half.
